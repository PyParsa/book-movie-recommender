# ==========================================================
# 🎬 Book & Movie Recommender App (Final Version)
...
def get_tmdb_genres():
    """دریافت لیست ژانرهای TMDB (id → name)"""
...
def discover_movies(genre_id: int, year: int, page: int = 1):
    """فیلم‌ها را بر اساس ژانر و سال می‌آورد"""
...
def search_books_by_subject(subject: str, year: int, max_results: int = 20):
    """کتاب‌ها را از Google Books بر اساس موضوع می‌آورد"""
...
if category == "🎥 Movies":
    if not TMDB_API_KEY:
        st.error("⚠️ TMDB API key not found. Please add it to your .env file.")
    else:
        try:
    ...
    if not GOOGLE_BOOKS_API_KEY:
        st.error("⚠️ Google Books API key not found. Please add it to your .env file.")
    else:
        ...
        if st.button("🎲 Recommend Books"):
            with st.spinner("Fetching from Google Books..."):
                try:
                    ...
                    if not results:
                        st.warning("No books found for this subject/year. Showing similar ones…")
                        results = search_books_by_subject(subject, 0, max_results=40)
                    picks = results[:top_n]
                    ...
                    st.error(f"Error fetching from Google Books: {e}")
